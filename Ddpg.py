...
class Ddpg:
    ...
    def __init__(self, state_dim, action_dim, actor_hid, critic_hid, actor_lr=1e-4, critic_lr=1e-3,
        ...
        self.Target_Actor = ActorNN(input_size=state_dim, output_size=action_dim, hidden_layer_sizes=actor_hid).to(self.device)
        ...
        self.Target_Critic = CriticNN(input_size=state_dim + action_dim, output_size=action_dim, hidden_layer_sizes=critic_hid).to(self.device)

        # self.Target_Critic = copy.deepcopy(self.Critic).to(self.device)

        # # The 4 Networks
        # self.Actor = Actor(state_dim, action_dim, actor_hid[0]).to(self.device)
        # self.Target_Actor = Actor(state_dim, action_dim, actor_hid[0]).to(self.device)
        #
        # # self.Target_Actor = copy.deepcopy(self.Actor).to(self.device)
        # self.Critic = Critic(state_dim + action_dim, action_dim, critic_hid[0]).to(self.device)
        # self.Target_Critic = Critic(state_dim + action_dim, action_dim, critic_hid[0]).to(self.device)
        #
        # # self.Target_Critic = copy.deepcopy(self.Critic).to(self.device)
